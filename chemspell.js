  ...
  //console.log('spelling ' + spelling + ' line ' + line + ' final '+ final)
    ...
    if (symbolsReversed.indexOf(symbols[spelling[i]]) != -1) {
      ...
      number = (symbols.indexOf(symbols[spelling[i]])+1).toString();
            if (final) {
        console.log('1. ' + spelling[i])
        console.log('2. ' + symbols[spelling[i]])
        console.log('3. ' + (symbols.indexOf(symbols[spelling[i]])+1).toString())
    ...
    if (final) {
      console.log('reverse? ' + reverseClass)
      console.log('spelling[i] ' + spelling[i] + ' number ' + number)
    }
    var urlVars = getUrlVars()
    if (symbolsReversed.indexOf(symbols[spelling[i]]) != -1) {
    ...
    var infourl = "info.html"+makeUrlVars(urlVars)
		table = table+"<td class=\"number turnWhite tableColor "+reverseClass+ " " + whitespaceClass + "\">"+"<a href=\""+infourl+"\">"+number+"</a></td>";
	}
	table = table+"</tr><tr>"
	for (i=0; i < spelling.length; i++) {
    reverseClass = ""
    ...
    if (symbolsReversed.indexOf(symbols[spelling[i]]) != -1) {
      ...
      urlVars["e"] = names[spelling[i]]
		...
		table = table+"<td class=\"symbol turnWhite tableColor "+reverseClass+" " + whitespaceClass +"\">"+"<a href=\""+infourl+"\">"+symbol+"</a></td>";
	}
	table = table+"</tr><tr>"
	for (i=0; i < spelling.length; i++) {
    reverseClass = ""
    ...
    if (curName == 'Whitespace') {
      curName = '';
      whitespaceClass = 'whitespace';
    }

		table = table+"<td class=\"name turnWhite tableColor "+reverseClass+" "+ whitespaceClass +"\">"+"<a href=\""+infourl+"\">"+curName+"</a></td>";
	}
	table = table + '</tr>'
  if (final) {
    console.log('spelling ' + spelling + ' line ' + line + ' final '+ final)
...
function elementSpell() {
  ...
  console.log('len ' + symbolsOrigLower.length)
	words = word.split(" ");
  console.log(words)
